...
static void currentCheckHV()
{
    ...
    int currentWatch = getHVsensorCurrent(); // this function transforms the read voltage to the main battery current in amps
...
static void voltageCheckHV()
{
    ...
    // global variables in Current_transducer.h store these ADC voltage and accumulator current such that HVcurrent_Range_Check can read them
    simulateVoltageHVADC(6000); // this function uses mibspi3 to simulate the behavior of the ADS7044 ADC that reads HV voltage
