...
void testMessage1()
{
    ...
    tx_data3[1] = maxCurrent;
//    tx_data3[2] = 256; // BYTE5 Control but crappy but works
    tx_data3[2] = 0;

    uint16 *tx_ptr3 = &tx_data3[0];

    int j = 0;
    for (j = 0; j < 4; j++)
    {
        // can't find a workaround to the uint16, the DBC needs it
        canTransmit(canREG1, canMESSAGE_BOX9, tx_ptr3);
        tx_ptr3 += 16; /* next chunk ...*/
    }
}
